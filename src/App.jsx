  ...
  return (
    <span style={{ position: "relative", display: "inline-flex", marginLeft: 5 }}>
      ...
      {open && <>
        ...
        <span style={{ position: "absolute", bottom: "calc(100% + 6px)", left: "50%", transform: "translateX(-60%)", background: "var(--tip)", color: "var(--tipT)", padding: "9px 12px", borderRadius: 9, fontSize: 11, lineHeight: 1.5, width: 220, zIndex: 100, boxShadow: "0 6px 24px rgba(0,0,0,.2)" }}>{text}</span>
      </>}
    </span>
  );
};
...
// ═══════════ Calculator Steps (Updated with Property, Multiple Gold & Business Equipment) ═══════════
const STEPS = [
  { id: "cash", t: "Cash & Savings", tb: "নগদ ও সঞ্চয়", ic: "Cash", c: "#10B981",
    fields: [
      ...
      { k: "salDue", l: "Salary/Wages Due", lb: "বকেয়া বেতন", tip: "Unpaid salary owed to you." }
    ] },
  ...
  { id: "inv", t: "Investments", tb: "বিনিয়োগ", ic: "Inv", c: "#8B5CF6",
    fields: [
      ...
      { k: "somiti", l: "Cooperative / Somiti", lb: "সমিতি", tip: "Cooperative society savings." },
      { k: "insurance", l: "Insurance Cash Value", lb: "বীমা নগদ মূল্য", tip: "Surrenderable cash value." },
      { k: "otI", l: "Other (Crypto, Bonds)", lb: "অন্যান্য", tip: "Cryptocurrency, bonds, insurance." },
    ] },
  { id: "biz", t: "Business & Trade", tb: "ব্যবসা", ic: "Biz", c: "#EC4899",
    fields: [
      { k: "bizC", l: "Business Cash", lb: "ব্যবসায়ের নগদ", tip: "Cash held for business." },
      { k: "invt", l: "Inventory / Stock", lb: "মজুদ পণ্য", tip: "Market value of goods for sale. NOT machinery." },
      { k: "rcv", l: "Receivables", lb: "পাওনা", tip: "Money owed to you." },
      { k: "bizVeh", l: "Business Vehicles", lb: "ব্যবসায়িক গাড়ি", tip: "CNG, truck, rickshaw for income. NOT personal car." },
      { k: "bizMach", l: "Income from Machinery", lb: "যন্ত্রপাতি আয়", tip: "Machinery itself NOT zakatable. Enter SAVED INCOME from it." }
    ] },
  { id: "prop", t: "Property & Land", tb: "সম্পত্তি ও জমি", ic: "Prop", c: "#06B6D4",
    note: "Personal home NOT zakatable. Only investment property/land.",
    noteBn: "ব্যক্তিগত বাসস্থান যাকাতযোগ্য নয়। শুধু বিনিয়োগের সম্পত্তি।",
    fields: [
      { k: "invPlot", l: "Investment Land/Plot", lb: "বিনিয়োগের জমি", tip: "Land bought to sell for profit." },
      { k: "invFlat", l: "Investment Flat", lb: "বিনিয়োগের ফ্ল্যাট", tip: "Flat bought to resell. NOT personal home." },
      { k: "rentSaved", l: "Saved Rental Income", lb: "জমানো ভাড়া", tip: "Accumulated rental income." },
      { k: "otProp", l: "Other Asset Income", lb: "অন্যান্য সম্পদ আয়", tip: "Warehouse, godown income etc." }
    ] },
...
const JARIYAH = [
  { id: "auto", name: "Auto-Rickshaw", nameBn: "অটো-রিকশা", cost: 350000, income: 800, emoji: "🛺",
    desc: "Gift an auto-rickshaw. Recipient earns ~৳800/day, pays back monthly.", descBn: "অটো-রিকশা দিন। প্রতিদিন ~৳৮০০ আয়, মাসিক কিস্তিতে পরিশোধ।" },
  { id: "sew", name: "Sewing Machine", nameBn: "সেলাই মেশিন", cost: 25000, income: 300, emoji: "🧵",
    desc: "Industrial sewing machine. ~৳300/day tailoring income.", descBn: "সেলাই মেশিন। দৈনিক ~৳৩০০ আয়।" },
  { id: "van", name: "Electric Van", nameBn: "ইলেকট্রিক ভ্যান", cost: 180000, income: 600, emoji: "🔋",
    desc: "Battery-powered delivery van. ~৳600/day earnings.", descBn: "ব্যাটারি ভ্যান। দৈনিক ~৳৬০০ আয়।" },
  { id: "shop", name: "Grocery Shop", nameBn: "মুদি দোকান", cost: 80000, income: 500, emoji: "🏪",
    desc: "Setup a small grocery shop. ~৳500/day profit.", descBn: "ছোট মুদি দোকান। দৈনিক ~৳৫০০ লাভ।" },
  { id: "cow", name: "Dairy Cow", nameBn: "দুধের গাভী", cost: 120000, income: 400, emoji: "🐄",
    desc: "Dairy cow yields ~৳400/day from milk sales.", descBn: "দুধের গাভী। দৈনিক ~৳৪০০ আয়।" },
  { id: "fish", name: "Fish Farming", nameBn: "মাছ চাষ", cost: 60000, income: 350, emoji: "🐟",
    desc: "Pond fish farming. ~৳350/day average.", descBn: "পুকুরে মাছ চাষ। দৈনিক গড়ে ~৳৩৫০।" },
];
...
export default function App() {
  ...
  // ── Multi-Karat Gold Logic ──
  const addGoldEntry = () => {
    const n = [...goldEntries, { karat: "22K", amount: 0, unit: "bhori" }];
    setGoldEntries(n); store.set("zf-gold", n);
  };
  const removeGoldEntry = (i) => {
    if (goldEntries.length <= 1) return;
    const n = goldEntries.filter((_, j) => j !== i);
    setGoldEntries(n); store.set("zf-gold", n);
  };
  const updateGoldEntry = (i, field, value) => {
    const n = [...goldEntries];
    n[i] = { ...n[i], [field]: value };
    setGoldEntries(n); store.set("zf-gold", n);
  };
  const getGoldGrams = (entry) => {
    const a = entry.amount || 0;
    return entry.unit === "gram" ? a : entry.unit === "bhori" ? a * BHORI : a * (BHORI / 16);
  };

  const totalGoldValue = useCallback(() => goldEntries.reduce((s, e) => s + getGoldGrams(e) * (GOLD_RATES[e.karat]?.perGram || 0), 0), [goldEntries]);
  const totalGoldGrams = useCallback(() => goldEntries.reduce((s, e) => s + getGoldGrams(e), 0), [goldEntries]);
  const getSilverGrams = useCallback(() => silverUnit === "bhori" ? silverAmt * BHORI : silverAmt, [silverAmt, silverUnit]);
  const silverValue = useCallback(() => getSilverGrams() * SILVER_RATE, [getSilverGrams]);

  // ── Calculate totals ──
  const calc = useCallback(() => {
    const v = vals;
    const goldVal = totalGoldValue();
    const slvVal = silverValue();
    ...
    const totalAssets = cash + goldVal + slvVal + invest + biz + prop;
    ...
    return { cash, goldVal, slvVal, invest, biz, prop, totalAssets, totalDebts, net, nisab, eligible, stdZakat, agriZakat, total: stdZakat + agriZakat, goldEntries: [...goldEntries] };
  }, [vals, totalGoldValue, silverValue, nisabStd, goldEntries]);
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  @media(min-width:600px){.ni{padding:6px 16px;font-size:11px}}
  ...
  if (view === "login") return (<><style>{css}</style>
    <div className="Z"><div style={{ minHeight: "100vh", padding: "60px 24px 40px", display: "flex", flexDirection: "column", maxWidth: 480, margin: "0 auto" }}>
      ...
      {!loginMethod ? <>
        <button onClick={() => setLoginMethod("google")} style={{ width: "100%", padding: "14px 20px", borderRadius: 12, border: "1.5px solid var(--bd)", background: "var(--cd)", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", marginBottom: 10, fontFamily: "inherit", fontSize: 14, fontWeight: 600, color: "var(--t1)" }}><IC.Google />Continue with Google</button>
        <button onClick={() => setLoginMethod("phone")} style={{ width: "100%", padding: "14px 20px", borderRadius: 12, border: "1.5px solid var(--bd)", background: "var(--cd)", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", marginBottom: 10, fontFamily: "inherit", fontSize: 14, fontWeight: 600, color: "var(--t1)" }}><IC.Phone />Continue with Phone</button>
      </> : <div className="fi">
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="text" value={loginName} onChange={(e) => setLoginName(e.target.value)} placeholder="Name" style={{ width: "100%", padding: "14px 16px", border: "1.5px solid var(--bd)", borderRadius: 11, fontSize: 15, background: "var(--inp)", color: "var(--t1)", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} autoFocus onKeyDown={(e) => e.key === "Enter" && doLogin()} />
        </div>
        <button className="b bp" style={{ width: "100%", padding: 15, fontSize: 15 }} onClick={doLogin}>Get Started <IC.Right /></button>
      </div>}
      <div style={{ flex: 1 }} />
  ...
  const Header = () => (
    <div className="hd">
      <div className="lg">{view !== "home" && <button onClick={() => go("home")} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex", color: "var(--t1)" }}><IC.Left /></button>}<span className="dt" /> ZakatFlow</div>
      <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
        <button onClick={toggleDark} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--t2)", display: "flex", padding: 3 }}>{dark ? <IC.Sun /> : <IC.Moon />}</button>
        <button className={`pl ${lang === "en" ? "on" : ""}`} onClick={() => setLang("en")}>EN</button>
        <button className={`pl ${lang === "bn" ? "on" : ""}`} onClick={() => setLang("bn")}>বাং</button>
      </div>
    </div>
  ...
  const VHome = () => (<div className="fi">
    ...
    <div className="cd" style={{ background: "linear-gradient(135deg,#0B7A62,#06805A)", color: "#fff", border: "none" }}>
      <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: .8, opacity: .6, marginBottom: 8 }}>NISAB • BAJUS • {RATES_DATE}</div>
    ...
    {curZakat && curZakat.total > 0 && <div className="cd" onClick={() => go("tracker")} style={{ cursor: "pointer" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><span style={{ fontSize: 11, fontWeight: 700, color: "var(--t2)" }}>{curZakat.year} ZAKAT</span><span className="tg" style={{ background: progress >= 1 ? "#E6F9F0" : "var(--goldL)", color: progress >= 1 ? "var(--ok)" : "var(--gold)" }}>{progress >= 1 ? "✓ DONE" : `${Math.round(progress * 100)}%`}</span></div>
      <div className="pr"><div className="prf" style={{ width: `${progress * 100}%` }} /></div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--t3)", marginTop: 6 }}><span>Paid: {fmt(totalPaid)}</span><span>Left: {fmt(remaining)}</span></div>
      <div className="mn" style={{ textAlign: "center", fontSize: 20, fontWeight: 800, color: "var(--ac)", marginTop: 6 }}>{fmt(curZakat.total)}</div>
      <div style={{ textAlign: "center", fontSize: 11, color: "var(--t3)", marginTop: 4 }}>≈ {fmt(monthly)}/mo × 12</div>
    </div>}
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
      ...
      <button className="b bo" style={{ width: "100%", fontSize: 12 }} onClick={() => go("jariyah")}>🤝 {bn ? "সদকা জারিয়া" : "Sadaqah Jariyah"}</button>
    </div>
    {years.length > 0 && <div style={{ marginTop: 18 }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--t3)", letterSpacing: .7, marginBottom: 8 }}>YEARLY HISTORY</div>
      {years.slice(0, 5).map((y, i) => <div key={i} className="cd" style={{ padding: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div><div style={{ fontSize: 14, fontWeight: 800 }}>{y.year}</div><div style={{ fontSize: 11, color: "var(--t3)" }}>Assets: {fmt(y.totalAssets)}</div></div>
        <div style={{ textAlign: "right" }}><div className="mn" style={{ fontSize: 16, fontWeight: 800, color: y.eligible ? "var(--ac)" : "var(--t3)" }}>{fmt(y.total)}</div><span className="tg" style={{ background: y.eligible ? "var(--acL)" : "var(--dangerL)", color: y.eligible ? "var(--ac)" : "var(--danger)" }}>{y.eligible ? "DUE" : "N/A"}</span></div>
      </div>)}</div>}
  </div>);
  ...
  const VCalc = () => {
    ...
    return (<div className="fi">
      ...
      {step === 0 && <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {[{ k: "silver", l: bn ? "রূপা" : "Silver", v: fmt(NISAB_SILVER_G * SILVER_RATE) }, { k: "gold", l: bn ? "স্বর্ণ" : "Gold", v: fmt(NISAB_GOLD_G * GOLD_RATES["22K"].perGram) }].map((n) => <div key={n.k} onClick={() => setNisabStd(n.k)} style={{ flex: 1, padding: 10, borderRadius: 10, border: `1.5px solid ${nisabStd === n.k ? "var(--ac)" : "var(--bd)"}`, cursor: "pointer", textAlign: "center", background: nisabStd === n.k ? "var(--acL)" : "var(--cd)" }}><div style={{ fontSize: 12, fontWeight: 700 }}>{n.l}</div><div className="mn" style={{ fontSize: 10, color: "var(--t3)" }}>{n.v}</div></div>)}
      </div>}
      ...
      {/* ── Dynamic Gold Form ── */}
      {s.isGold ? <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--t2)", marginBottom: 8 }}>{bn ? "আপনার স্বর্ণ (একাধিক ক্যারেট যোগ করুন)" : "YOUR GOLD — Add multiple karats"}</div>
        {goldEntries.map((entry, i) => (
          <div key={i} className="ge">
            {goldEntries.length > 1 && <button onClick={() => removeGoldEntry(i)} style={{ position: "absolute", top: 8, right: 8, background: "var(--dangerL)", border: "none", borderRadius: 6, padding: "4px 8px", cursor: "pointer", color: "var(--danger)", fontSize: 10, fontWeight: 700, fontFamily: "inherit" }}>✕</button>}
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--gold)", marginBottom: 6 }}>{bn ? `এন্ট্রি ${i + 1}` : `Entry ${i + 1}`}</div>
            <div className="kg">
              {Object.entries(GOLD_RATES).map(([k, v]) => (
                <div key={k} className={`ko ${entry.karat === k ? "on" : ""}`} onClick={() => updateGoldEntry(i, "karat", k)}>
                  <div style={{ fontWeight: 800, fontSize: 11 }}>{k === "Sonaton" ? (bn ? "সনাতন" : "Trad.") : k}</div>
                  <div style={{ fontSize: 9, color: "var(--t3)" }}>৳{v.perGram.toLocaleString()}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 5, marginBottom: 8, alignItems: "center" }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: "var(--t3)" }}>Unit:</span>
              {[{ k: "bhori", l: bn ? "ভরি" : "Bhori" }, { k: "gram", l: "Gram" }, { k: "ana", l: bn ? "আনা" : "Ana" }].map((u) => <span key={u.k} className={`pl ${entry.unit === u.k ? "on" : ""}`} onClick={() => updateGoldEntry(i, "unit", u.k)}>{u.l}</span>)}
            </div>
            <Inp label={bn ? "পরিমাণ" : "Amount"} value={entry.amount} onChange={(v) => updateGoldEntry(i, "amount", v)} suffix={entry.unit === "bhori" ? "ভরি" : entry.unit === "ana" ? "আনা" : "gm"} />
            {entry.amount > 0 && <div style={{ background: "var(--goldL)", borderRadius: 8, padding: "6px 12px", display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--gold)" }}>
              <span>{entry.amount} {entry.unit} × {entry.karat}</span><span className="mn" style={{ fontWeight: 800 }}>{fmt(getGoldGrams(entry) * GOLD_RATES[entry.karat].perGram)}</span>
            </div>}
          </div>
        ))}
        <button className="b bg" style={{ width: "100%", marginBottom: 14 }} onClick={addGoldEntry}><IC.Plus /> {bn ? "আরেকটি ক্যারেট যোগ" : "Add Another Gold Type"}</button>
        
        {totalGoldValue() > 0 && <div style={{ background: "var(--goldL)", borderRadius: 10, padding: "10px 14px", marginBottom: 12, textAlign: "center" }}>
          <div style={{ fontSize: 10, color: "var(--gold)", fontWeight: 600 }}>TOTAL GOLD VALUE</div>
          <div className="mn" style={{ fontSize: 18, fontWeight: 800, color: "var(--gold)" }}>{fmt(totalGoldValue())}</div>
          <div style={{ fontSize: 10, color: "var(--gold)", opacity: .7 }}>{totalGoldGrams().toFixed(1)}gm from {goldEntries.filter(e => e.amount > 0).length} entries</div>
        </div>}
        
        <div style={{ borderTop: "1px solid var(--bd)", paddingTop: 14, marginTop: 4 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--t2)", marginBottom: 8 }}>SILVER</div>
          <div style={{ display: "flex", gap: 5, marginBottom: 8, alignItems: "center" }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: "var(--t3)" }}>Unit:</span>
            {[{ k: "gram", l: "Gram" }, { k: "bhori", l: "Bhori" }].map((u) => <span key={u.k} className={`pl ${silverUnit === u.k ? "on" : ""}`} onClick={() => setSilverUnit(u.k)}>{u.l}</span>)}
          </div>
          <Inp label={bn ? "রূপা" : "Silver Amount"} value={silverAmt} onChange={(v) => setSilverAmt(v)} suffix={silverUnit === "bhori" ? "ভরি" : "gm"} tip="Nisab: 612.36g" />
        </div>
      </div> 
      : <div className="cd">
        {s.fields.map((fd) => <Inp key={fd.k} label={bn ? fd.lb : fd.l} value={vals[fd.k]} onChange={(v) => sv(fd.k, v)} tip={fd.tip} suffix="৳" />)}
      </div>}
      ...
      <div style={{ background: "var(--inp)", borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", justifyContent: "space-between" }}>
        ...
        <span className="mn" style={{ fontSize: 14, fontWeight: 800, color: t.net >= 0 ? "var(--ac)" : "var(--danger)" }}>{fmt(t.net)}</span>
      </div>
      
      <div style={{ display: "flex", gap: 8 }}>
        {step > 0 && <button className="b bg" onClick={() => setStep(step - 1)} style={{ flex: 1 }}><IC.Left />Back</button>}
        {step < STEPS.length - 1 ? <button className="b bp" onClick={() => setStep(step + 1)} style={{ flex: 2 }}>Next <IC.Right /></button>
          : <button className="b bp" onClick={doCalc} style={{ flex: 2, background: "linear-gradient(135deg,var(--ac),#059669)" }}>Calculate <IC.Check /></button>}
      </div>
    </div>);
  };
  ...
  // Result
  const VResult = () => {
    if (!curZakat) return null;
    const z = curZakat;
    return (<div className="fi" style={{ textAlign: "center" }}>
      <div style={{ padding: "14px 0" }}>
        <div style={{ width: 50, height: 50, borderRadius: 14, background: z.eligible ? "var(--acL)" : "var(--dangerL)", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
          {z.eligible ? <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--ac)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg> 
        ...
        <h2 style={{ fontSize: 16, fontWeight: 700 }}>{z.eligible ? "Zakat is Due" : "Not Due"}</h2>
      </div>
      {z.eligible && <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 10, color: "var(--t3)", fontWeight: 600 }}>TOTAL ZAKAT</div>
        <div className="mn" style={{ fontSize: 34, fontWeight: 800, color: "var(--ac)", letterSpacing: -2 }}>{fmt(z.total)}</div>
        <div style={{ fontSize: 12, color: "var(--t3)", marginTop: 4 }}>≈ {fmt(z.total / 12)}/mo × 12</div>
        {z.agriZakat > 0 && <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 2 }}>(Std: {fmt(z.stdZakat)} + Agri: {fmt(z.agriZakat)})</div>}
      </div>}
      <div className="cd" style={{ textAlign: "left" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--t2)", marginBottom: 10 }}>BREAKDOWN</div>
        {[{ l: "Cash", v: fmt(z.cash) }, { l: "Gold", v: fmt(z.goldVal), c: "var(--gold)" },
          ...(z.goldEntries || []).filter(e => e.amount > 0).map(e => ({ l: `  ${e.karat} (${e.amount} ${e.unit})`, v: fmt(getGoldGrams(e) * GOLD_RATES[e.karat].perGram), s: 1, c: "var(--gold)" })),
          { l: "Silver", v: fmt(z.slvVal), s: 1 }, { l: "Investments", v: fmt(z.invest) }, { l: "Business", v: fmt(z.biz) }, { l: "Property", v: fmt(z.prop) },
          { l: "Total Assets", v: fmt(z.totalAssets), b: 1 }, { l: "Debts", v: `-${fmt(z.totalDebts)}`, c: "var(--danger)" },
          { l: "Net", v: fmt(z.net), b: 1, c: "var(--ac)" }, { l: "Nisab", v: fmt(z.nisab) }
        ].map((r, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: r.s ? "2px 0 2px 16px" : "6px 0", borderTop: i && !r.s ? "1px solid var(--bd)" : "none", fontSize: r.s ? 11 : 12, color: r.s ? "var(--t3)" : "var(--t1)", fontWeight: r.b ? 700 : 400 }}>
          <span>{r.l}</span><span className="mn" style={{ fontWeight: 600, color: r.c }}>{r.v}</span>
        </div>)}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {z.eligible && <button className="b bp" style={{ width: "100%" }} onClick={() => go("tracker")}><IC.Bar />Track Payments</button>}
        <div style={{ display: "flex", gap: 8 }}>
          <button className="b bg" style={{ flex: 1 }} onClick={() => { setStep(0); go("calc"); }}>Redo</button>
          <button className="b bo" style={{ flex: 1 }} onClick={shareResult}><IC.Share />Share</button>
        </div>
      </div>
    </div>);
  };
  ...
  const VMonthly = () => (<div className="fi">
    <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 2 }}>Monthly Plan</h2>
    <p style={{ color: "var(--t3)", fontSize: 11, marginBottom: 14 }}>Split across 12 months</p>
    {(!curZakat || curZakat.total <= 0) ? <div style={{ textAlign: "center", padding: "40px 16px", color: "var(--t3)" }}>
      <p>Calculate first</p>
      <button className="b bp" style={{ marginTop: 12 }} onClick={() => { setStep(0); go("calc"); }}>Calculate</button>
    </div> : <>
      <div className="cd" style={{ textAlign: "center" }}><div style={{ fontSize: 10, color: "var(--t3)", fontWeight: 600 }}>MONTHLY TARGET</div><div className="mn" style={{ fontSize: 28, fontWeight: 800, color: "var(--ac)", marginTop: 2 }}>{fmt(monthly)}</div></div>
      <div className="g3">{MONTHS.map((m, i) => {
        const pd = monthPaid(i), dn = pd >= monthly, pt = pd > 0;
        return <div key={i} className="cd" style={{ padding: 10, textAlign: "center", borderColor: dn ? "var(--ac)" : pt ? "var(--gold)" : "var(--bd)", background: dn ? "var(--acL)" : pt ? "var(--goldL)" : "var(--cd)" }}>
          <div style={{ fontSize: 11, fontWeight: 700 }}>{m}</div>
          <div className="mn" style={{ fontSize: 12, fontWeight: 700, color: dn ? "var(--ac)" : pt ? "var(--gold)" : "var(--t3)" }}>{fmt(pd)}</div>
          <div style={{ fontSize: 9, color: "var(--t3)" }}>{dn ? "✓" : `/${fmt(monthly)}`}</div>
        </div>
      })}</div>
      <button className="b bp" style={{ width: "100%", marginTop: 8 }} onClick={() => go("tracker")}><IC.Plus />Add Payment</button>
    </>}
  </div>);

  // Tracker
  const VTracker = () => (<div className="fi">
    <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 2 }}>Payment Tracker</h2>
    {curZakat && curZakat.total > 0 && <div className="cd" style={{ textAlign: "center" }}>
      <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 10 }}>
        <div><div style={{ fontSize: 9, color: "var(--t3)", fontWeight: 600 }}>DUE</div><div className="mn" style={{ fontSize: 16, fontWeight: 800 }}>{fmt(curZakat.total)}</div></div>
        <div style={{ width: 1, background: "var(--bd)" }} />
        <div><div style={{ fontSize: 9, color: "var(--t3)", fontWeight: 600 }}>LEFT</div><div className="mn" style={{ fontSize: 16, fontWeight: 800, color: remaining > 0 ? "var(--gold)" : "var(--ok)" }}>{fmt(remaining)}</div></div>
      </div>
      <div className="pr"><div className="prf" style={{ width: `${progress * 100}%` }} /></div>
    </div>}
    {!showPayForm ? <button className="b bp" style={{ width: "100%", marginBottom: 12 }} onClick={() => setShowPayForm(true)}><IC.Plus />Add Payment</button>
      : <div className="cd fi" style={{ border: "1.5px solid var(--ac)" }}>
        <Inp label="Amount" value={newPay.amount} onChange={(v) => setNewPay((p) => ({ ...p, amount: v }))} suffix="৳" />
        <div style={{ marginBottom: 12 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: "var(--t2)", marginBottom: 4, display: "block" }}>Note</label>
          <input type="text" value={newPay.note} onChange={(e) => setNewPay((p) => ({ ...p, note: e.target.value }))} placeholder="e.g. Mosque" style={{ width: "100%", padding: "11px 13px", border: "1.5px solid var(--bd)", borderRadius: 9, fontSize: 13, background: "var(--inp)", color: "var(--t1)", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
        </div>
        <div style={{ display: "flex", gap: 8 }}><button className="b bg" style={{ flex: 1 }} onClick={() => setShowPayForm(false)}>Cancel</button><button className="b bp" style={{ flex: 1 }} onClick={addPayment}><IC.Check />Save</button></div>
      </div>}
    {payments.length > 0 && <div className="cd">
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--t2)", marginBottom: 6 }}>LOG ({payments.length})</div>
      {payments.map((p) => <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--bd)" }}>
        <div><div className="mn" style={{ fontSize: 13, fontWeight: 700 }}>{fmt(p.amount)}</div><div style={{ fontSize: 10, color: "var(--t3)" }}>{p.note && `${p.note} • `}{new Date(p.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div></div>
        <button className="b bd" onClick={() => delPayment(p.id)}><IC.Trash /></button>
      </div>)}
    </div>}
    {payments.length === 0 && <div style={{ textAlign: "center", padding: "32px", color: "var(--t3)" }}>📋 No payments yet</div>}
  </div>);

  // Fitrana
  const VFitrana = () => (<div className="fi">
    <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 2 }}>Fitrana Calculator</h2>
    <div className="cd">
      <label style={{ fontSize: 11, fontWeight: 600, color: "var(--t2)", marginBottom: 6, display: "block" }}>Family Members</label>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <button className="b bg" style={{ padding: "8px 14px" }} onClick={() => setFitraPpl(Math.max(1, fitraPpl - 1))}>−</button>
        <span className="mn" style={{ fontSize: 26, fontWeight: 800, minWidth: 36, textAlign: "center" }}>{fitraPpl}</span>
        <button className="b bg" style={{ padding: "8px 14px" }} onClick={() => setFitraPpl(fitraPpl + 1)}>+</button>
      </div>
      <div style={{ background: "var(--inp)", borderRadius: 10, padding: 14, textAlign: "center" }}>
        <div style={{ fontSize: 10, color: "var(--t3)", fontWeight: 600 }}>TOTAL FITRANA</div>
        <div className="mn" style={{ fontSize: 28, fontWeight: 800, color: "var(--ac)", marginTop: 2 }}>{fmt(fitraPpl * FITRA_RATE)}</div>
        <div style={{ fontSize: 10, color: "var(--t3)" }}>{fitraPpl} × ৳{FITRA_RATE}</div>
      </div>
    </div>
  </div>);
  ...
  const VJariyah = () => (<div className="fi">
    <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 2 }}>🤝 Sadaqah Jariyah</h2>
    <p style={{ color: "var(--t3)", fontSize: 11, marginBottom: 14 }}>Give someone a means of earning</p>
    {!selJariyah ? <div className="g2">
      {JARIYAH.map((j) => <div key={j.id} className="cd" style={{ padding: 14, cursor: "pointer", textAlign: "center" }} onClick={() => setSelJariyah(j)}>
        <div style={{ fontSize: 28, marginBottom: 4 }}>{j.emoji}</div>
        <div style={{ fontSize: 13, fontWeight: 700 }}>{bn ? j.nameBn : j.name}</div>
        <div className="mn" style={{ fontSize: 12, fontWeight: 700, color: "var(--ac)", marginTop: 2 }}>{fmt(j.cost)}</div>
        <div style={{ fontSize: 10, color: "var(--t3)" }}>~৳{j.income}/day</div>
      </div>)}
    </div> : <div className="fi">
      <button onClick={() => setSelJariyah(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--t2)", display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 600, marginBottom: 12, fontFamily: "inherit" }}><IC.Left />All</button>
      <div className="cd" style={{ textAlign: "center" }}>
        <div style={{ fontSize: 40, marginBottom: 6 }}>{selJariyah.emoji}</div>
        <h3 style={{ fontSize: 18, fontWeight: 800 }}>{bn ? selJariyah.nameBn : selJariyah.name}</h3>
        <p style={{ fontSize: 12, color: "var(--t2)", marginTop: 4 }}>{bn ? selJariyah.descBn : selJariyah.desc}</p>
      </div>
      <div className="cd">
        {[{ l: "Cost", v: fmt(selJariyah.cost), b: 1 }, { l: "Daily", v: `~৳${selJariyah.income}` }, { l: "Monthly", v: `~${fmt(selJariyah.income * 26)}` }, { l: "Payback", v: `~${Math.ceil(selJariyah.cost / (selJariyah.income * 26))} mo` }].map((r, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: i ? "1px solid var(--bd)" : "none", fontSize: 12, fontWeight: r.b ? 700 : 400 }}>
          <span>{r.l}</span><span className="mn" style={{ fontWeight: 700, color: "var(--ac)" }}>{r.v}</span>
        </div>)}
      </div>
    </div>}
  </div>);
  ...
  return (<><style>{css}</style>
    <div className="Z">
      ...
      <div className="nv"><div className="nvI">
        {[{ id: "home", l: bn ? "হোম" : "Home", i: <IC.Home /> }, { id: "calc", l: bn ? "হিসাব" : "Calc", i: <IC.Cash /> }, { id: "monthly", l: bn ? "মাসিক" : "Monthly", i: <IC.Cal /> }, { id: "tracker", l: bn ? "ট্র্যাক" : "Track", i: <IC.Bar /> }, { id: "jariyah", l: bn ? "জারিয়া" : "Jariyah", i: <IC.Heart /> }].map((n) => <button key={n.id} className={`ni ${view === n.id || (view === "result" && n.id === "calc") ? "on" : ""}`} onClick={() => { if (n.id === "calc") setStep(0); go(n.id); }}>{n.i}{n.l}</button>)}
      </div></div>
    </div>
  </>);
}