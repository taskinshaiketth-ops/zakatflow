...
export default function App() {
  ...
  const doManualCalc = () => {
    ...
    const prof = { id: Date.now(), name: profName, zakatDue: r.total, payments: [], date: new Date().toISOString(), year: yr, result: r, isQuick: true };
    const updP = [prof, ...profiles]; saveProfiles(updP); setActiveProfile(prof);
    const rec = { ...r, year: yr, date: new Date().toISOString() };
    const updY = [...years.filter(y => y.year !== yr), rec].sort((a, b) => b.year - a.year);
    setYears(updY); LS.set("zf-yrs", updY);
  ...
  const VHome = () => <div className="fi">
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
      <button className="b bp" style={{ width: "100%", padding: "15px 20px", fontSize: 14 }} onClick={() => { setStep(0); setCalcMode(null); setVals({}); setGoldEntries([{ karat: "22K", amount: 0, unit: "bhori" }]); setSilverAmt(0); setCalcName(""); go("calc"); }}>{bn ? "যাকাত হিসাব করুন" : "Calculate Zakat"} <IC.Right /></button>
  ...
  const VCalc = () => {
    ...
    if (calcMode === "manual") return <div className="fi">
      ...
      <div className="cd" style={{ padding: 14, marginBottom: 12, border: calcName.trim() ? "1.5px solid var(--ac)" : "1.5px solid var(--gold)" }}>
        ...
        <input type="text" value={calcName} onChange={e => setCalcName(e.target.value)} placeholder={bn ? "যেমন: আমার যাকাত" : "e.g. My Zakat"} style={{ width: "100%", padding: "11px 13px", border: "1.5px solid var(--bd)", borderRadius: 9, fontSize: 13, background: "var(--inp)", color: "var(--t1)", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
      </div>
  ...
  const VTrk = () => {
      ...
      {profilesForYear.length > 0 ? profilesForYear.map(p => {
        ...
        return <div key={p.id} className="cd" style={{ padding: 14, cursor: "pointer" }} onClick={() => { setActiveProfile(p); setTrackerSubView("tracker"); }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <div><div style={{ fontSize: 13, fontWeight: 700 }}>{p.name}</div><div style={{ fontSize: 10, color: "var(--t3)" }}>{new Date(p.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}{p.isQuick && " • Manual"}</div></div>
    ...
    return <div className="fi">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><div><h2 style={{ fontSize: 16, fontWeight: 800 }}>{activeProfile.name}</h2><div style={{ fontSize: 10, color: "var(--t3)" }}>{activeProfile.isQuick ? "Manual" : "Calculated"} • {new Date(activeProfile.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div></div><div style={{ display: "flex", gap: 6 }}><button className="b bg" style={{ fontSize: 10, padding: "7px 10px" }} onClick={() => { setActiveProfile(null); setTrackerSubView("tracker"); }}>{bn ? "পরিবর্তন" : "Switch"}</button><button className="b bd" style={{ fontSize: 10 }} onClick={() => { saveProfiles(profiles.filter(p => p.id !== activeProfile.id)); setActiveProfile(null); go("home"); }}><IC.Trash /> Del</button></div></div>
