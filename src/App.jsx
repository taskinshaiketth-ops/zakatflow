...
export default function App() {
  ...
  const doCalc = () => {
    ...
    const prof = { id: Date.now(), name: profName, zakatDue: r.total, payments: [], date: new Date().toISOString(), year: yr, result: r };
  ...
  const VHome = () => <div className="fi">
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
      <button className="b bp" style={{ width: "100%", padding: "15px 20px", fontSize: 14 }} onClick={() => { setStep(0); setVals({}); setGoldEntries([{ karat: "22K", amount: 0, unit: "bhori" }]); setSilverAmt(0); setCalcName(""); go("calc"); }}>{bn ? "যাকাত হিসাব করুন" : "Calculate Zakat"} <IC.Right /></button>
  ...
  // ═══════════ TRACKER ═══════════
  const VTrk = () => { if (!activeProfile) return <div className="fi">
    <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>{bn ? "পেমেন্ট ট্র্যাকার" : "Payment Tracker"}</h2>
    <p style={{ color: "var(--t3)", fontSize: 11, marginBottom: 14 }}>{bn ? "একটি প্রোফাইল নির্বাচন করুন" : "Select a profile to track payments"}</p>
    {profiles.length > 0 ? profiles.map(p => {
      ...
      return <div key={p.id} className="cd" style={{ padding: 14, cursor: "pointer" }} onClick={() => setActiveProfile(p)}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          <div><div style={{ fontSize: 13, fontWeight: 700 }}>{p.name}</div><div style={{ fontSize: 10, color: "var(--t3)" }}>{new Date(p.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}{p.isQuick && " • Manual"}</div></div>
