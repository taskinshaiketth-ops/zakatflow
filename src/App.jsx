...
export default function App() {
  ...
  const [calcProfileName, setCalcProfileName] = useState("");
  ...
  const ProfileList = ({ onSelect }) => (
    <>
      {profiles.map(p => {
        ...
        return (
          <div key={p.id} className="cd" style={{ padding: 14, cursor: "pointer" }} onClick={() => onSelect(p)}>
  ...
  const doCalc = () => {
    ...
    const finalName = calcProfileName.trim() || ((bn ? "যাকাত " : "Zakat ") + yr);
    
    const prof = { id: Date.now(), name: finalName, zakatDue: r.total, payments: [], date: new Date().toISOString(), year: yr, result: r };
    ...
    setCalcProfileName(""); // reset input
    go("result");
  };
    ...
    <div className="fi">
      ...
      <div className="cd" style={{ background: "linear-gradient(135deg,#0B7A62,#06805A)", color: "#fff", border: "none" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div><div style={{ fontSize: 11, opacity: .75 }}>{bn ? "স্বর্ণ ৭.৫ভরি" : "Gold 7.5 Bhori"}</div><div className="mn" style={{ fontSize: 17, fontWeight: 800 }}>{formatCurrency(NISAB_GOLD_GRAMS * goldRates["22K"].g)}</div></div>
          <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, opacity: .75 }}>{bn ? "রূপা ৫২.৫তোলা" : "Silver 52.5 Tola"}</div><div className="mn" style={{ fontSize: 17, fontWeight: 800 }}>{formatCurrency(NISAB_SILVER_GRAMS * silverRate)}</div></div>
        </div>
        <div style={{ fontSize: 8, opacity: .35, marginTop: 6, textAlign: "center" }}>{bn ? "উৎস: gold-api.com (আন্তর্জাতিক স্পট) × open.er-api.com (BDT রেট) × BD প্রিমিয়াম" : "Source: gold-api.com (intl spot) × open.er-api.com (BDT rate) × BD retail premium"}</div>
      </div>
      ...
      {showNewProfile && (
        <div className="cd fi" style={{ border: "1.5px solid var(--ac)", marginTop: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}><button className="b bg" style={{ flex: 1 }} onClick={() => setShowNewProfile(false)}>{bn ? "বাতিল" : "Cancel"}</button><button className="b bp" style={{ flex: 1 }} onClick={createQuickProfile}><IC.Check />{bn ? "ট্র্যাক শুরু" : "Start Tracking"}</button></div>
        </div>
      )}

      {profiles.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--t3)", letterSpacing: .7, display: "flex", alignItems: "center", gap: 4 }}><IC.Users /> {bn ? "প্রোফাইলসমূহ" : "PROFILES"}</div><span style={{ fontSize: 10, color: "var(--t3)" }}>{profiles.length}</span></div>
          <ProfileList onSelect={(p) => { setActiveProfile(p); go("tracker"); }} />
        </div>
      )}

      <div style={{ marginTop: 20, borderTop: "2px dashed var(--bd)", paddingTop: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}><span style={{ fontSize: 16 }}>🤲</span><div><div style={{ fontSize: 12, fontWeight: 700, color: "var(--jar)" }}>{bn ? "সদকা জারিয়া (স্বেচ্ছা দান)" : "Sadaqah Jariyah (Voluntary)"}</div><div style={{ fontSize: 10, color: "var(--danger)", fontWeight: 600 }}>⚠ {bn ? "এটি যাকাত নয়!" : "This is NOT Zakat!"}</div></div></div>
      ...
      <div className="fi">
        ...
        {s.isGold ? (
          <div>
            ...
            {goldEntries.map((entry, i) => (
              <div key={i} className="ge">
                ...
                <div className="kg">
                  {Object.entries(goldRates).map(([k, v]) => <div key={k} className={`ko ${entry.karat === k ? "on" : ""}`} onClick={() => upGE(i, "karat", k)}><div style={{ fontWeight: 800, fontSize: 11 }}>{k === "Sonaton" ? (bn ? "সনাতন" : "Trad.") : k}</div><div style={{ fontSize: 9, color: "var(--t3)" }}>৳{v.g.toLocaleString()}</div></div>)}
                </div>
                <div style={{ display: "flex", gap: 5, marginBottom: 8, alignItems: "center" }}><span style={{ fontSize: 10, fontWeight: 700, color: "var(--t3)" }}>Unit:</span>{[{ k: "bhori", l: bn ? "ভরি" : "Bhori" }, { k: "gram", l: "Gram" }, { k: "ana", l: bn ? "আনা" : "Ana" }].map(u => <span key={u.k} className={`pl ${entry.unit === u.k ? "on" : ""}`} onClick={() => upGE(i, "unit", u.k)}>{u.l}</span>)}</div>
                ...
                {entry.amount > 0 && <div style={{ background: "var(--goldL)", borderRadius: 8, padding: "6px 12px", display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--gold)" }}><span>{entry.amount} {entry.unit} × {entry.karat}</span><span className="mn" style={{ fontWeight: 800 }}>{formatCurrency(g2g(entry) * goldRates[entry.karat].g)}</span></div>}
              </div>
            ))}
          ...
          <div className="cd">
            ...
            {/* New Profile Name Input on Last Step */}
            {step === STEPS.length - 1 && (
              <div style={{ marginTop: 16 }}>
                <label style={{ display: "flex", alignItems: "center", fontSize: 12, fontWeight: 600, color: "var(--t2)", marginBottom: 4 }}>
                ...
                <input 
                  type="text" 
                  value={calcProfileName} 
                  onChange={(e) => setCalcProfileName(e.target.value)} 
                  placeholder={bn ? "যেমন: আমার যাকাত ২০২৬" : "e.g. My Zakat 2026"} 
                  style={{ width: "100%", padding: "12px 14px", border: "1.5px solid var(--bd)", borderRadius: 10, fontSize: 14, fontFamily: "inherit", background: "var(--inp)", color: "var(--t1)", outline: "none", boxSizing: "border-box" }}
                  onFocus={(e) => { e.target.style.borderColor = "var(--ac)"; e.target.style.boxShadow = "0 0 0 3px var(--acG)"; }}
                  onBlur={(e) => { e.target.style.borderColor = "var(--bd)"; e.target.style.boxShadow = "none"; }}
                />
              </div>
  ...
  const VTrk = () => { 
    if (!activeProfile) {
      return (
        <div className="fi">
          <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 14 }}>{bn ? "প্রোফাইল নির্বাচন করুন" : "Select a Profile"}</h2>
          {profiles.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 16px", color: "var(--t3)" }}>
              <p>{bn ? "প্রথমে হিসাব করুন" : "Calculate Zakat first"}</p>
    ...
    return (
      <div className="fi">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div><h2 style={{ fontSize: 16, fontWeight: 800 }}>{activeProfile.name}</h2><div style={{ fontSize: 10, color: "var(--t3)" }}>{activeProfile.isQuick ? "Manual" : "Calculated"} • {new Date(activeProfile.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div></div>
        ...
        <div className="cd" style={{ textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 10 }}><div><div style={{ fontSize: 9, color: "var(--t3)", fontWeight: 600 }}>DUE</div><div className="mn" style={{ fontSize: 16, fontWeight: 800 }}>{formatCurrency(activeProfile.zakatDue)}</div></div><div style={{ width: 1, background: "var(--bd)" }} /><div><div style={{ fontSize: 9, color: "var(--t3)", fontWeight: 600 }}>LEFT</div><div className="mn" style={{ fontSize: 16, fontWeight: 800, color: profRem > 0 ? "var(--gold)" : "var(--ok)" }}>{formatCurrency(profRem)}</div></div></div>
  ...
  const VMon = () => {
    ...
    return (
      <div className="fi">
        ...
        <div className="cd" style={{ textAlign: "center" }}><div style={{ fontSize: 10, color: "var(--t3)", fontWeight: 600 }}>MONTHLY TARGET</div><div className="mn" style={{ fontSize: 28, fontWeight: 800, color: "var(--ac)", marginTop: 2 }}>{formatCurrency(profMon)}</div><div style={{ fontSize: 11, color: "var(--t3)" }}>{activeProfile.name}</div></div>
        <div className="g3">{MO.map((m, i) => { const pd = moPd(i), dn = pd >= profMon, pt = pd > 0; return <div key={i} className="cd" style={{ padding: 10, textAlign: "center", borderColor: dn ? "var(--ac)" : pt ? "var(--gold)" : "var(--bd)", background: dn ? "var(--acL)" : pt ? "var(--goldL)" : "var(--cd)" }}><div style={{ fontSize: 11, fontWeight: 700 }}>{m}</div><div className="mn" style={{ fontSize: 12, fontWeight: 700, color: dn ? "var(--ac)" : pt ? "var(--gold)" : "var(--t3)" }}>{formatCurrency(pd)}</div></div>; })}</div>
  ...
  const renderGuide = () => <div className="fi"><h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 2 }}>{bn ? "যাকাত গাইড" : "Zakat Guide"}</h2><p style={{ color: "var(--t3)", fontSize: 11, marginBottom: 14 }}>{bn ? "সম্পূর্ণ যাকাত নির্দেশিকা" : "Complete Zakat guide for Bangladeshi Muslims"}</p>{GUIDE.map((sec) => { const isOpen = openGuide === sec.id; return <div key={sec.id} className="cd" style={{ padding: 0, overflow: "hidden", marginBottom: 8, cursor: "pointer" }} onClick={() => setOpenGuide(isOpen ? null : sec.id)}><div style={{ padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}><div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 20 }}>{sec.icon}</span><span style={{ fontSize: 13, fontWeight: 700 }}>{bn ? sec.tb : sec.t}</span></div><span style={{ fontSize: 12, color: "var(--t3)" }}>{isOpen ? "▲" : "▼"}</span></div>{isOpen && <div style={{ padding: "0 16px 16px", borderTop: "1px solid var(--bd)" }}><div style={{ fontSize: 12, lineHeight: 1.7, color: "var(--t2)", marginTop: 12, whiteSpace: "pre-line" }}>{bn ? sec.bn : sec.en}</div><div style={{ marginTop: 8, fontSize: 10, color: "var(--ac)", fontStyle: "italic" }}>📖 {sec.ref}</div></div>}</div>; })}<div style={{ marginTop: 12, borderTop: "2px dashed var(--bd)", paddingTop: 16 }}><h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>🌙 {bn ? "ফিতরা ক্যালকুলেটর" : "Fitrana Calculator"}</h3><div className="cd"><div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}><button className="b bg" style={{ padding: "8px 14px" }} onClick={(e) => { e.stopPropagation(); setFitraN(Math.max(1, fitraN - 1)); }}>−</button><span className="mn" style={{ fontSize: 26, fontWeight: 800, minWidth: 36, textAlign: "center" }}>{fitraN}</span><button className="b bg" style={{ padding: "8px 14px" }} onClick={(e) => { e.stopPropagation(); setFitraN(fitraN + 1); }}>+</button></div><div style={{ background: "var(--inp)", borderRadius: 10, padding: 14, textAlign: "center" }}><div className="mn" style={{ fontSize: 28, fontWeight: 800, color: "var(--ac)" }}>{formatCurrency(fitraN * fitraRate)}</div><div style={{ fontSize: 10, color: "var(--t3)" }}>{fitraN} × ৳{fitraRate}</div></div><div style={{ marginTop: 10 }}><Inp label={bn ? "ফিতরা রেট (প্রতিজন)" : "Fitra Rate/person"} value={fitraRate} onChange={v => { setFitraRate(v); LS.set("zf-fitra", v); }} suffix="৳" tip={bn ? "ইসলামিক ফাউন্ডেশনের হার অনুযায়ী" : "Per Islamic Foundation rate"} /></div></div></div></div>;
        ...
        <div className="nv">
          <div className="nvI">
            {[
              ...
              { id: "guide", l: bn ? "গাইড" : "Guide", i: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg> }
            ].map(n => (
